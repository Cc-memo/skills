...
- For a known project, the project probe performs a read-only freshness check against the watched repository/files. If it reports `stale`, run `project_freshness.py --scan`, review the change, and only then build a focus-specific `project_context.py` pack. Accept a new baseline only after review; stale context is not authoritative.
...
- Route prior research, comparisons, choices, rollback reasons, and retired approaches through `recall_solution.py --route decision`. Route a named project continuation through `--route project --project "<project>"` and load a focused project context pack only when requested by the probe.
- The problem cue queries `_系统/problem-solution-index.json`, which contains only trusted solved problems and stable/verified playbooks. The decision route queries `_系统/decision-index.json`, which contains compact trusted decision evidence. A reviewed delivery may enter the latter only with explicit `decision_index: true` or a strong decision/research classification. Rebuild both with `python scripts/build_memory_indexes.py`.
- Exact error signatures receive the strongest match; symptoms, technologies, aliases, tags, project, and root cause provide secondary matches. After selecting an answer, load its full card by `record_id`; consult sessions only as supporting evidence.
- If an index is missing or invalid, recall falls back to a deterministic lexical scan constrained to the same record class. Decision fallback never promotes all sessions into candidate answers. A fallback is not semantic retrieval and should be reported when relevant.
- Indexes include a lightweight source manifest and are written atomically. A changed record invalidates the cached index; the problem route may use the lexical fallback and directs maintenance to `build_memory_indexes.py`.
