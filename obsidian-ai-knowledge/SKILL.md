...
- The cue stage first queries `_系统/problem-solution-index.json`, a compact machine-readable directory containing only trusted solved problems and stable/verified playbooks. It does not scan sessions as primary answers. Rebuild it with `python scripts/build_solution_index.py` after promoting, updating, or superseding a problem/playbook.
...
- If the index is missing or invalid, recall falls back to the existing deterministic lexical scan and keeps unverified records in a separate clue section. A fallback is not semantic retrieval and should be reported when relevant.
